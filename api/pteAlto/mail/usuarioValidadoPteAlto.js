/**
 * Email de notificación cuando un usuario es validado en Puente Alto.
 * Indica que puede agendar espacios deportivos e inscribirse en talleres y eventos.
 */
...
const sendUsuarioValidadoPteAlto = async (usuario) => {
  try {
    ...
    const html = `
        ...
        <head>
          ...
          <title>Cuenta validada - Deportes Puente Alto</title>
        ...
        <body>
          <div class="container">
            ...
            <div class="body-content">
              <p class="welcome-title">¡Tu cuenta ha sido validada!</p>
              <p class="greeting">Hola <strong>${nombre} ${apellido}</strong>,</p>
              <p class="greeting">Te informamos que tu cuenta en la plataforma de la Corporación de Deportes Puente Alto ha sido validada y habilitada correctamente.</p>
              <div class="datos-box">
                <p class="datos-title">Ya puedes:</p>
              ...
              <p class="greeting">Ingresa a la plataforma para comenzar a reservar y disfrutar de nuestras actividades deportivas.</p>
    ...
    const { error } = await resend.emails.send({
      ...
      to: email,
      subject: "Tu cuenta ha sido validada - Deportes Puente Alto",
      html,
    });

    if (error) {
      throw new Error(
        `Resend no pudo enviar el correo de usuario validado: ${error.message || "Error desconocido"}`
      );
    }

    console.log("Email de usuario validado enviado correctamente a:", email);
  } catch (error) {
    console.error("Error enviando email de usuario validado:", error);
    throw error;
  }
};

module.exports = sendUsuarioValidadoPteAlto;
