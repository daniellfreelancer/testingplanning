...
/**
 * Envía email de confirmación de reserva con QR code (Resend)
 * @param {String} email - Email destinatario
 * @param {String} nombreDestinatario - Nombre para el saludo (usuario o organización)
 * @param {Object} reserva - Reserva populada (espacioDeportivo/taller; usuario solo si aplica)
 * @param {Object} [options]
 * @param {boolean} [options.esParaOrganizacion] - true: reserva interna hacia club (no exige reserva.usuario)
 */
const sendEmailReservaPteAlto = async (email, nombreDestinatario, reserva, options = {}) => {
    try {
        if (!EMAIL_SERVICE_API_RESEND) {
            throw new Error("EMAIL_SERVICE_API_RESEND no está configurada");
        }

        const esParaOrganizacion = options.esParaOrganizacion === true;

        if (!email || !String(email).trim()) {
            console.log("Sin email destinatario, no se envía confirmación");
            return;
        }

        if (!reserva || !reserva._id) {
            console.log("Reserva sin ID, no se envía email");
            return;
        }

        if (!esParaOrganizacion) {
            if (!reserva.usuario) {
                console.log("Reserva sin usuario, no se envía email");
                return;
            }
        ...
        const nombreCompleto = nombreDestinatario || (esParaOrganizacion ? "Organización" : "Usuario");
        const introTexto = esParaOrganizacion
            ? "La reserva interna registrada a nombre de su organización ha sido confirmada. Detalle:"
                ...
                <table class="content-table" align="center" justify="center" role="presentation" cellspacing="0" cellpadding="0" border="0">
                  ...
                  <tr>
                    <td class="body-content">
                      ...
                      <p class="greeting-text">${introTexto}</p>
                      
                      <!-- Reservation Details -->
                      <div class="reservation-details">
                        ${reserva.esReservaInterna && reserva.tipoReservaInterna ? `
                        <div class="detail-row">
                          <span class="detail-label">Clase de reserva interna:</span>
                          <span class="detail-value">${String(reserva.tipoReservaInterna)}</span>
                        </div>
                        ` : ""}
                        ${reserva.reservadoPara ? `
                        <div class="detail-row">
                          <span class="detail-label">Reservado para:</span>
                          <span class="detail-value">${String(reserva.reservadoPara)}</span>
                        </div>
                        ` : ""}
                        <div class="detail-row">
        ...
        if (error) {
            throw new Error(
                `Resend no pudo enviar el correo de confirmación de reserva (organización): ${error.message || "Error desconocido"}`
            );
        }
